...
BMP bilateralFilter(const BMP& in,int kernelSize,double spaceSigma,double colorSigma){
    ...
    int maxColor=256;
    vector<double>colorWeights(maxColor);
        ...
        for(int y=radius;y<width-radius;y++){
            ...
            for(int i=-radius;i<=radius;i++){
                for(int j=-radius;j<=radius;j++){
                    ...
                    int colorDist=abs(q.r-pixel.r)+abs(q.b-pixel.b)+abs(q.g-pixel.g);
                    //color distance of pixel(i,j) and pixel(x+i,y+j)
                    double weight=colorWeights[colorDist]*spaceWeights[offset];
                    sumr+=q.b*weight;
                    sumg+=q.g*weight;
                    sumb+=q.b*weight;
                    sumw+=weight;
            ...
            out.setRGB(x,y,rgb(BMP::solve(sumr),BMP::solve(sumb),BMP::solve(sumb)));
        }
    }
    return out;
}